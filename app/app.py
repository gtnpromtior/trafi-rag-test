...
def update_vectorstore_incrementally(
    vector_store, folder_id, current_state, previous_state
):
    ...
    try:
        ...
        for file_id, current_file in current_files.items():
            if (
                file_id not in previous_files
                or current_file["modified_time"]
                != previous_files[file_id]["modified_time"]
            ):
                    ...
                    if documents:
                        ...
                        try:
                            results = vector_store.similarity_search_with_score(
                                "dummy query", k=1000, filter={"file_id": file_id}
                            )
                            if results:
                                doc_ids_to_delete = [
                                    doc[0].metadata.get("doc_id")
                                    for doc in results
                                    if doc[0].metadata.get("doc_id")
                                ]
                                if doc_ids_to_delete:
                                    vector_store.delete(ids=doc_ids_to_delete)
                        except Exception as delete_error:
                            st.warning(
                                f"Could not delete old versions: {str(delete_error)}"
                            )
        ...
        if removed_ids:
            for file_id in removed_ids:
                ...
                try:
                    ...
                    st.warning(
                        f"Could not delete file {file_info['name']}: {str(delete_error)}"
                    )
